---
title: Offline Data Analysis of *Strongyloides venezuelensis* bulk RNA-seq data
output:
  html_document:
    code_folding: hide
    df_print: paged
...
# Introduction
The goal of this file is to lay out proof-of-concept analyses of the *Strongyloides venezuelensis* RNA-seq dataset originally published by [Hunt *et al* 2018](https://www.nature.com/articles/s41598-018-23514-z).  
...
```{r loadBaseData}
# Load and Parse Preprocessed Data
load (file = "../Data/SvRNAseq_group_FLF_PF_data_preprocessed")
...
p1<-dend %>% 
  ...
  labs(title = "S. venezuelensis: Hierarchical Cluster Dendrogram",
       subtitle = "filtered, TMM normalized, batch FLF/PF",
       y = "Distance",
       x = "Life stage") +
  coord_fixed(1/2) +
...
p3<-ggplot(pca.res.df) +
  ...
  labs(title="S. venezuelensis: Principal Components Analysis of RNA-seq Samples",
       caption = "Note: analysis is blind to life stage identity.",
       subtitle ="RNA-seq Dataset: batch FLF_PF",
       fill = "Life Stage",
       shape = "Life Stage") +
...
p<-pheatmap(diffGenes,
            ...
            scale = "row",
            angle_col = 45,
            main = "Sv: Log2 Counts Per Million (CPM) Expression Across Life Stages (Batch FLF_PF)"
...
print(paste('Total number of genes in Str-RNA-seq Browser PF vs FLF output file:', nrow(Browser.dat))) 
...
p.benchmark <- ggplot(plotting.all, aes(x = logFC.x, y = logFC.y)) +
  ...
  labs(title = "S. venezuelensis: Str-Browser vs Hunt Data",
       subtitle = "Batch FLF_PF; comparison = PF vs FLF",
...
suppressMessages(ggsave("Sv_CountComparisons.pdf",
                        plot = p.cpm_all,
                        device = "pdf",
                        height = 5,
                        width = 6,
...
load(file = "../Data/Sv_allSamples_vDGEList")
