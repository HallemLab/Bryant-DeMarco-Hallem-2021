...
```{r loadBaseData}
load (file = "../Data/SsRNAseq_data_preprocessed")
...
suppressMessages(ggsave("Ss_CountComparisons.pdf",
                        plot = p.cpm_all,
                        device = "pdf",
                        height = 5,
                        width = 6,
...
cpm.merged <- rbind(cpm.all %>% dplyr::mutate(condition = "Both"),
  ...
  group_by(geneID, condition, life_stage)

cpm.merged.avg <- cpm.merged %>%
  dplyr::summarise(expressionAVG = median(expression))

options(contrasts = c("contr.sum", "contr.poly"))
res.aov2<- aov(expressionAVG ~ condition * life_stage, data = cpm.merged.avg)
...
### Comparing LogFC between Browser and Akimori Dataset

```{r benchmarkingAkimori}
...
# Load Akimori Dataset: FLM vs FLF comparison
temp.dat <-  read.xlsx ("../Data/Benchmarking/41598_2021_87478_MOESM3_ESM.xlsx", 
                        sheet = 1, startRow = 1)

Akimori.dat <- tibble(geneID = temp.dat$genes, logFC = temp.dat$log2.Fold.Change)%>%
...
print(paste('Total number of genes in Akimori *et al* 2021 FLM vs FLF comparison tab:',nrow(Akimori.dat)))
print(paste('Total number of genes in Str-RNA-seq Browser FLM vs FLF output file:', nrow(Browser.dat))) 


plotting.all <- inner_join(Browser.dat, Akimori.dat, by = "geneID")
plotting.BrowserOnly <- anti_join(Browser.dat, Akimori.dat, by = "geneID")

# The plot below takes the genes with LogFC results in both the Browser and Akimori databases, and plots the two sets against each other. 
linearMod <- lm(logFC.y ~ logFC.x, data = plotting.all) %>%
  summary()
...
p.benchmarkFLM <- ggplot(plotting.all, aes(x = logFC.x, y = logFC.y)) +
  geom_smooth(method=lm, color = 'red', formula = "y ~ x") +
  geom_point(shape=16, size=3, alpha = 0.8) +
  labs(title = "S. stercoralis: Str-Browser vs Akimori Data",
       subtitle = "FLM vs FLF",
       caption = paste("points = genes; red line/shading = linear regression \n",
                       "w/ 95% confidence regions (formula = y ~ x). \n",
                       "Adj R-squared =",
...
print("Linear regression of Browser vs Akimori LogFC results:")
(linearMod)
...
### PC1 Loadings
```{r GSEA.PC1.Loadings, message=FALSE}
# Generate rank ordered list of genes ----
# Use pca.res$rotation to select genes influencing PC1-6 ----
myscores.df <- pca.res$rotation[,1:6] %>% 
  as_tibble(rownames = "geneID") %>%
...
### PC2 Loadings
```{r GSEA.PC2.Loadings, message=FALSE}
# Generate rank ordered list of genes ----
Top10.GSEA.sub <- myscores.df %>%
  dplyr::filter(PC == "PC2") %>%
  dplyr::select(!abs_scores) 
...
suppressMessages(ggsave("Ss_PCid_GSEA.pdf",
                        plot = p.PC.gsea,
                        device = "pdf",
                        height = 7,
                        #width = 7,
