...
Imports study design file and Kallisto reads into the R environment. At this stage, we separate the two experimental batches, such that differences in library size and transcript length between the two batches are not corrected when calculating counts from abudnance (we use the lengthScaledTPM option in `tximport`). This code chunk generates and saves an object containing the transcripts per million data. In subsequent chunks, that file is loaded, and analysis progresses.
```{r txImport}
# load packages ----
suppressPackageStartupMessages({
  library(tidyverse) 
...
# Get first out-subclade group homologs for S. venezuelensis genes 
# from BioMart and filter -----
...
# Get second out-subclade group homologs for S. venezuelensis genes 
# from BioMart and filter -----
...
Collecting code for reading in Kallisto reads through generating a DGEList. Gene annotations are not re-imported. This code chunk generates and saves an object containing the transcripts per million data. In subsequent chunks, that file is loaded, and analysis progresses. 
```{r iL3_extended.preprocessing.1}
...
```{r iL3.extended.preprocessing.8}
...
## Import Kallisto Reads  
This code chunk generates and saves an object containing the transcripts per million data. In subsequent chunks, that file is loaded, and analysis progresses. 
